...
| POST | `/api/auth/refresh` | JWT（接受过期） | 无感刷新 Token，返回新 JWT |
...
```js
// JWT 签发
signToken(payload) → String  // 7 天有效期

// JWT 验证
verifyToken(token) → decoded  // 严格验证（检查过期）
verifyTokenRelaxed(token) → decoded  // 宽松验证（忽略过期，用于刷新端点）

// 认证中间件
requireAuthMw(req, res, next)  // 验证 Bearer Token，注入 req.user

// 管理员中间件
requireAdminMw(req, res, next)  // requireAuth + 校验 role === 'admin'

// 可选认证（不拦截未登录用户）
optionalAuthMw(req, res, next)  // 有 token 则注入 req.user，无则 req.user = null
```
...
const ALLOWED_ORIGINS = [
  'https://pikachuprogrammer01.github.io',  // 生产环境（GitHub Pages）
  'https://pikachu01.me',                   // 生产环境（自定义域名）
  'https://myblog-api-five.vercel.app',     // API 自身（Vercel）
...
- JWT Token 7 天过期，支持 `/api/auth/refresh` 无感刷新（接受过期令牌，签发新令牌）
- 前端 401 拦截器自动刷新 Token，刷新失败才跳转登录页
- CORS 白名单限制允许的来源
- 数据库连接使用 TLS 加密
